...
export class MessageService implements OnModuleInit {
    ...
    async create(createMessageDTO: CreateMessageDTO): Promise<MessageDTO> {
        const message = this.messageMapper.dtoToEntity(
            createMessageDTO as MessageDTO,
        );
    ...
    async findInboxByEmail(email: string): Promise<MessageDTO[]> {
        ...
        const messages = await this.entityManager
            ...
            .andWhere(
                "(message.to = :email OR :email = ANY(string_to_array(message.cc, ',')))",
                {
                    email,
                },
            )
    ...
    async findMessagesByTab(
        ...
        const userMessages = allMessagesForUser.filter((message) => {
            ...
            const isInTo =
                Array.isArray(message.to) && message.to.includes(email);
            // Check if email is in the 'cc' array
            const isInCc =
                Array.isArray(message.cc) && message.cc.includes(email);
            return isInTo || isInCc;
        });

        console.log(
            `Found ${userMessages.length} messages for user ${email}:`,
            userMessages.map((m) => ({
                id: m.id,
                to: m.to,
                from: m.from,
        ...
        switch (tab) {
            ...
            case "read":
                ...
                break;
            case "vip":
                filteredMessages = userMessages.filter(
                    (m) => m.from && m.from.toLowerCase().includes("vip"),
                );
                break;
            default:
        ...
        console.log(
            `Found ${filteredMessages.length} ${tab} messages:`,
    ...
    async update(
        id: number,
        updateMessageDTO: Partial<MessageDTO>,
    ): Promise<MessageDTO> {
