...
async getOrCreateConversation(userId: number, otherUserId: number): Promise<ChatConversation> {
    ...
    const existing = await this.conversationsRepo
      ...
      .where(qb => {
        const sub = qb.subQuery()
          ...
          .where('cp.userId IN (:...ids)', { ids: [userId, otherUserId] })
    ...
    return await this.conversationsRepo.manager.transaction(async (trx) => {
      const conversation = await trx.getRepository(ChatConversation).save(trx.getRepository(ChatConversation).create({}));
      const participants = [
        trx.getRepository(ChatParticipant).create({ conversationId: conversation.id, userId: a, unreadCount: 0 }),
        trx.getRepository(ChatParticipant).create({ conversationId: conversation.id, userId: b, unreadCount: 0 }),
      ];
  ...
  async getConversationsForUser(userId: number) {
    ...
    const conversations = await this.conversationsRepo.createQueryBuilder('c')
      .innerJoin('c.participants', 'pf', 'pf.userId = :userId', { userId })
    ...
    for (const conv of conversations) {
      const others = (conv as any).participants?.filter((pp: any) => pp.userId !== userId) || [];
      if (others.length > 0) {
        const other = others[0];
        // Lazy-load user via participant.user relation if available
        if (!other.user) {
          other.user = await this.participantsRepo.createQueryBuilder('cp')
