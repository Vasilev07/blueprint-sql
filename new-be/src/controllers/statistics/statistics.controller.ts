...
export class StatisticsController {
    ...
    @ApiQuery({ name: "timeRange", required: false, description: "Time range (e.g., '20h', '7d'). If omitted, defaults to Gifts Today only." })
    ...
    async getDashboardStatistics(@Query("timeRange") timeRange?: string) {
        try {
            return await this.statisticsService.getDashboardStatistics(timeRange);
        } catch (error: any) {
