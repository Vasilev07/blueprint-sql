...
export class StoriesService {
  ...
  private initializeMockData() {
    const mockStories: Story[] = [
      ...
      },
      {
        ...
        caption: 'Hiking in the mountains 🏔️ #hiking #mountains #adventure',
        duration: 18,
        views: 2034,
        likes: 156,
        comments: 31,
  ...
  uploadStory(storyData: Omit<Story, 'id' | 'createdAt' | 'expiresAt' | 'views' | 'likes' | 'comments' | 'isLiked' | 'isViewed'>): Observable<Story> {
  ...
  getExpiredStories(): Story[] {
    const now = new Date();
    return this.storiesSubject.value.filter(story => story.expiresAt < now);
  }

  cleanupExpiredStories(): void {
    const validStories = this.storiesSubject.value.filter(story => story.expiresAt > new Date());
