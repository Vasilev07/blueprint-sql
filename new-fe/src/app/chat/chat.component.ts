...
export class ChatComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.chatService.friends$.pipe(takeUntil(this.destroy$)).subscribe(friends => {
        ...
        next: (conv) => {
          ...
          this.chatService.subscribeToConversation(conv.id)
            ...
            .subscribe((msg: any) => {
              ...
              (this.messages as any).push(mapped);
              this.scrollToBottom();
            });
        },
        error: () => {
  ...
  openConversation(conv: { id: any; participants?: any[] }) {
    ...
    const participants = (conv.participants || []).map((p: any) => String(p));
    const otherUserId = participants.find(p => p !== loggedId) || participants[0] || this.currentUserId;
    // Normalize to numeric user id if an email sneaks in from legacy data
    const targetUserId = /^(\d+)$/.test(String(otherUserId))
      ? String(otherUserId)
      : (this.allUsers.find(u => u.email === otherUserId)?.id?.toString() || this.currentUserId);
  ...
  getConversationName(conv: any): string {
    const loggedId = String(this.getLoggedInUserId());
    const otherId = (conv?.participants || []).map((p: any) => String(p)).find((p: string) => p !== loggedId) || '';
    const user = this.allUsers.find(u => String(u.id) === otherId || u.email === otherId);
  ...
  getOtherUserId(conv: { participants?: any[] }): string {
    ...
    const rawOther = parts.find((p: string) => p !== loggedId) || parts[0] || this.currentUserId;
  ...
  sendMessage(): void {
    if (this.messageForm.valid && this.currentUserId) {
      ...
      const convId = this.conversationId ? Number(this.conversationId) : undefined;
      this.chatService.sendChatMessage(convId, Number(this.currentUserId), content);
