...
export class ChatService {
  ...
  private loadMessagesAndConversations() {
    const email = this.authService.getUserEmail();
    const url = 'http://localhost:3000/messages/tab';
    const headers = this.getAuthHeaders();

    const loadTab = (tab: 'unread' | 'read' | 'vip') => this.httpClient.post<any[]>(url, { email, tab }, { headers });

    forkJoin({ unread: loadTab('unread'), read: loadTab('read') }).subscribe({
      next: ({ unread, read }) => {
        const all = [...(unread || []), ...(read || [])];
        const readIds = new Set((read || []).map((r: any) => r.id));

        const mapped: Message[] = all.map((m: any) => {
          const senderKey = String(m.fromEmail ?? m.from ?? m.fromId ?? '');
          const toField = Array.isArray(m.toEmail)
            ? m.toEmail[0]
            : (Array.isArray(m.to) ? m.to[0] : (m.toEmail ?? m.to ?? m.toId));
          const receiverKey = String(toField ?? '');
          return {
            id: String(m.id ?? `${senderKey}-${m.createdAt}`),
            senderId: senderKey,
            receiverId: receiverKey || email,
            content: m.content ?? m.subject ?? '',
            timestamp: new Date(m.createdAt ?? Date.now()),
            isRead: m.isRead !== undefined ? Boolean(m.isRead) : readIds.has(m.id),
            type: 'text'
          } as Message;
        });
        ...
        for (const msg of mapped) {
          const otherId = msg.senderId === email ? msg.receiverId : msg.senderId;
          const existing = conversationsMap.get(otherId);
          const conv: Conversation = {
            id: otherId,
            participants: [String(email), otherId],
          ...
          if (!existing || (existing.lastMessageTime || 0) < (msg.timestamp || 0)) {
  ...
  loadConversationMessages(conversationId: number): Observable<Message[]> {
    return this.httpClient.get<any[]>(`http://localhost:3000/chat/messages/${conversationId}`, { headers: this.getAuthHeaders() }) as any;
  }
